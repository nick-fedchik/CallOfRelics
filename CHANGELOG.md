...
- **GDD.md** (Draft 1.3 → 1.4) — Декомпозиція та реструктуризація
  - Об'єднано секції 2+5 (Глобальні стани + Персонаж) → скорочена секція 2
  - Об'єднано секції 6+7+8+9 (Корабель + Модулі + Орбіта + Сканування) → скорочена секція 5
  - Об'єднано секції 11-15 (Планети + Локації + Класифікація + Прогресія + Невдачі) → скорочена секція 7
  - Перенумеровано всі розділи послідовно (1-13)
  - Додано посилання на деталізовані документи

- **PLAYER.md** (v1.3 → v1.4) — Розширено контентом з GDD
  - Додано секцію 2 "Глобальні стани гравця" (stateDiagram)
  - Додано секцію 9 "Взаємодія гравця з системами" (UseCase diagram)
  - Додано journey діаграму "Шлях гравця"
  - Перенумеровано розділи (11 секцій)

- **SPACESHIP.md** (v1.7 → v1.8) — Розширено контентом з GDD
  - Додано секцію 8 "Орбітальний ігровий стан" (flowchart)
  - Додано секцію 9 "Системи сканування" (Activity diagram)
  - Перенумеровано розділи (11 секцій)

### Added - New Documentation

- **PLANET_LOCATIONS.md** (v1.0) — Новий документ системи локацій
  - Витяг секцій 11-15 з GDD з повними деталями
  - Mermaid діаграми: flowchart, stateDiagram, pie, quadrantChart, xychart-beta, gantt
...
Нова структура GDD.md (13 розділів замість 21):
